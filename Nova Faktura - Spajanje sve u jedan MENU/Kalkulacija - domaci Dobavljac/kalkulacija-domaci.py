...
def unos(*args):
    ...
    global redni_broj, suma, dobavljaceva_vrijednost, pdv_ulazni_suma,prodajna_vrijednost_suma, prodajna_bez_pdv_suma, LISTA_ZA_PRINTING, pdv_izlazni, LISTA_ZA_UNOS_U_BAZU
    pdv = 1.19
    JM = "KOM"
    #PDV = "19%"
    ...
    razlika_u_cijeni = prodajna_cijena_bez_pdv - cijena2
    ...
    mlb.insert(END, ('%d' % redni_broj, '%d' % sifra2, '%s' % naziv2, '%d' % komada2, JM, str(cijena2), str(cijena2), str(pdv_ulazni), str(razlika_u_cijeni), str(prodajna_cijena_bez_pdv), PDV, str(maloprodajna_cijena2)))
    
    #
    # Prosledjujuje HTML-u kreatoru da stvori file za printing
    #
    lista_za_ispis = [redni_broj, sifra2, naziv2, komada2, JM, cijena2, cijena2, pdv_ulazni, razlika_u_cijeni, prodajna_cijena_bez_pdv, PDV, maloprodajna_cijena2]
...
Label(root, text="Maloprodajna cijena").place(x = 100, y = 157)
...
mlb = MultiListbox(root, (('R. Broj', 2), ('Sifra artikla', 8), ('Naziv artikla', 26),('Komada', 7) ,('JM', 4), ('Dob. Cijena', 7),('Nab. Vrijednost', 7), ('PDV-ulazni', 7),  ('Razlika u cijeni', 12), 
...
def ispisi():
    ...
    f2.write('<div class="left"> <center> <strong>Dobavljac:  </strong><p> '+ str(dobavljac.get()) + '  </p> <p> PDV: '+ str(dobavljac_pdv.get()) +'</p> <p> PIB: '+ str(dobavljac_pib.get()) +'</p> <p> Tel: '+ str(dobavljac_telefon.get()) + '</p> <p> Adresa: ' + str(dobavljac_adresa.get()) +' </p> </div> </center>\n')
    ...
    zadnji_red = ['<strong>', '', '<strong>UKUPNO</strong>', '' ,'', '<strong>' + str(round(sum(dobavljaceva_vrijednost), 2)), 
                                                                     '<strong>' + str(round(sum(dobavljaceva_vrijednost), 2)), 
    ...
    htmlcode = HTML.table(LISTA_ZA_PRINTING, header_row = ['R.Broj',   'Sifra',   'Naziv artikla', 'Kolicina','JM',  'Dob. Cijena', 'Nab. cijena', 'PDV-ulazni', 'Raz. u cijeni', 'Prod. Cijena bez PDV', 'PDV', 'Prd. Cijena s PDV'],
                                             col_width=['10%', '10%', '25%', '10%', '10%','10%', '10%', '10%', '10%', '10%', '10%', '10%',] ,
                                             col_align=['center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center', 'center'], 
                                             col_styles=['font-size: 10px', 'font-size: 10px','font-size: 10px','font-size: 10px','font-size: 10px','font-size: 10px',
