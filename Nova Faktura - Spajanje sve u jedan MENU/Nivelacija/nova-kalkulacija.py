...
def ispisi():
    ...
    f2.write('<center><strong style="font-size: bog"> Promjena cijene</center></storng>')
    ...
    suma_bez_pdv.append(str(sum(cijene_bez_pdv)))
...
def onselect(evt):
    ...
    global cijene_bez_pdv, cijene_sa_pdv, suma_samo_pdv, redni_broj

    w = mlb
    index = int(w.curselection()[0])
    value = w.get(index)
    print index
    for x in LIST_OF_LISTBOXES:
        x.delete(index, index)
